...
namespace TechonlineAPI
{
    public class TOS : IDisposable
    {
        ...
        public bool isUserRegistred( String email )
        {
            ...
            using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM vishal_users WHERE email = @email"))
            {
        ...
        public User getUser(String email, bool includePassword = false)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT * FROM vishal_users WHERE email = @email"))
                {
                    cmd.Parameters.AddWithValue("@email", email);
                    cmd.Connection = connection;
        ...
        public bool register(String name, String lastname, String email, String password, String type)
        {
            ...
            String istr = "INSERT vishal_users (name, lastname, email, password, signup_date, type) VALUES(@name, @lastname, @email, @password, @now, @type)";
        ...
        public List<Brand> getAllBrands()
        {
            ...
            String sstr = "SELECT C.id, C.cat_name, COUNT(*) as total FROM vishal_categories C, vishal_products P WHERE C.id = P.category GROUP BY C.id, C.cat_name";
        ...
        {
            String query = "SELECT * FROM vishal_products, vishal_categories WHERE vishal_products.category=vishal_categories.id AND vishal_products.id=@id";
            Product p = null;
        ...
        public List<Product> getProducts(int brand)
        {
            ...
            if (brand > -1)
            {
                query = "SELECT * FROM vishal_products, vishal_categories WHERE vishal_products.category=vishal_categories.id AND vishal_products.category=@id";
            }
            else
            {
                query = "SELECT * FROM vishal_products, vishal_categories WHERE vishal_products.category=vishal_categories.id";
            }
            using (SqlCommand cmd = new SqlCommand(query, connection))
            {
        ...
        public bool insertOrder(int user_id, int product_id, int quantity, string payment_method, string shipping_address, string shipping_city, string shipping_postal, string shipping_country)
        {
            ...
            String istr = "INSERT INTO vishal_orders (user_id, product_id, quantity, payment_method, shipping_address, shipping_city, shipping_postal, shipping_country, date) VALUES(@user_id, @product_id, @quantity, @payment_method, @shipping_address, @shipping_city, @shipping_postal, @shipping_country, @date)";
            using (SqlCommand cmd = new SqlCommand(istr, connection))
            {
        ...
        {
            String query = "SELECT * FROM vishal_orders WHERE vishal_orders.user_id=@uid ORDER BY vishal_orders.date DESC";
        ...
        public bool ChangePassword(string user_email, string oldpassword, string newpassword)
        {
            ...
            {
                String istr = "UPDATE vishal_users SET password=@password WHERE email=@user_email";
                using (SqlCommand cmd = new SqlCommand(istr, connection))
                {
